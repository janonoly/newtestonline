...
from .models import PaperList, Question, Paper, CourseList
...
class PaperView(View):
    ...
    def get(self, request, paper_id):
        if request.user.is_authenticated:
            ...
            #科目
            # course_id=papers_list.course
            # 先获取到外键所在模型数据表的数据集对象
            cousreid = PaperList.objects.get(id=paper_id).course.id
            ...
            jd_score1 = papers_list.jd_choice_num * papers_list.jd_choice_score
            #随机试卷
            if papers_list.random_paper == 1:
                question_list = []
                xz_num = Question.objects.filter(questionType ='xz',course_id=cousreid).count()+1
                pd_num = Question.objects.filter(questionType = 'pd',course_id=cousreid).count()
                mxz_num = Question.objects.filter(questionType = 'mxz',course_id=cousreid).count()
                jd_num = Question.objects.filter(questionType='jd', course_id=cousreid).count()
                ...
                seq4 = [i for i in range(xz_num+pd_num+mxz_num, xz_num+pd_num+mxz_num+jd_num)]
                ...
                question_id_list4 = random.sample(seq4, papers_list.jd_choice_num)
                question_id_list1.extend(question_id_list2)
                question_id_list1.extend(question_id_list3)
                question_id_list1.extend(question_id_list4)
            ...
            for i in question_id_list:
                ...
                if temp_question.questionType == 'xz':
                    ...
                    if temp_question.answer == user_ans:
                        ...
                        question = Question.objects.get(pk=i)
                        wrong_question.append(question)
                elif temp_question.questionType == 'jd':
                    if temp_question.answer == user_ans.strip():
                        temp_score += papers_list.jd_choice_score
                        print("试题", temp_question.id, "答案正确")
                    else:
                        question = Question.objects.get(pk=i)
                        wrong_question.append(question)
...
class TrainView(View):
    ...
    def post(self, request,question_id):
        ...
        if question_now.questionType == 'xz':
            if question_now.answer == user_ans:
                ...
                return render(request, "train.html",
                              {"question_now": question_now, "pre_question_id": pre_question_id,
        ...
        elif question_now.questionType == 'jd':
            jd_ans = str(question_id) + '_jd'
            user_ans = request.POST.get(jd_ans, "")
            if question_now.answer.strip() == user_ans.strip():
                flag2 = 1
                return render(request, "train.html",
                              {"question_now": question_now, "pre_question_id": pre_question_id,
                               "next_question_id": next_question_id,
                               "question_count": question_count, "flag1": flag1,
                               "flag2": flag2, })
            else:
                flag2 = 0
                return render(request, "train.html",
...
class CourseTrainView(View):
    def get(self, request):
        project_data={}
        result= CourseList.objects.all().values_list()
        tihaostart=1
        for i in list(result):
            project_data[i[0]] = i[1]

        return render(request, "course_train.html", {'project': project_data,"tihaostart":tihaostart})
    ...
    def post(self, request):
        ...
        try:
            ...
            tihaostart= int(request.POST.get("tihaostart"))
            # tihaoend= int(request.POST.get( "tihaoend"))
            coursese_id = CourseList.objects.filter(name=course_name).values_list()[0][0]


        except:
            print("科目输入有误")
        if (tihaostart is not None  and question_now is not None):
            # return render(request, "train.html",{"tihaostart": tihaostart} )
            #question_count = Question.objects.filter(course_id=coursese_id).count()
            question_id=Question.objects.filter(course_id=coursese_id)
            id=question_id.values_list("id", flat=True)
            select_id_set=id[tihaostart-1:tihaostart+2]
            tihaostart=select_id_set[0]



            return  redirect('/exams/train/'+str(tihaostart))
        # 2.选择科目时

        else:
            # 1.未选择科目时
            qingxuanzhe = "请选择科目及题目范围"
            return render(request, "course_train.html")
